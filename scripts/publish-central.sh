#!/usr/bin/env bash
# Signs every published module with sbt-pgp, zips target/sona-staging and uploads the bundle to
# the Sonatype Central Portal, then polls until Maven Central has published it. Credentials come
# from SONATYPE_CENTRAL_USERNAME / SONATYPE_CENTRAL_PASSWORD or ~/.sbt/sonatype_central_credentials.
set -euo pipefail
...
if [[ "$VERSION" == *-SNAPSHOT ]]; then
  echo "Version ${VERSION} is a snapshot. Snapshots go to Central's snapshot repository with 'sbt --server publishSigned' and no bundle upload." >&2
  exit 1
fi

if [[ -z "$DEPLOYMENT_NAME" ]]; then
  DEPLOYMENT_NAME="com.anjunar.hibernateddl:hibernate-ddl-manager:${VERSION}"
fi
...
for ((attempt = 1; attempt <= MAX_POLLS; attempt++)); do
  ...
  case "$DEPLOYMENT_STATE" in
    ...
    FAILED)
      ...
      ;;
    VALIDATED)
      # Final only for USER_MANAGED, as in the PowerShell script; with AUTOMATIC publishing
      # it is a step on the way to PUBLISHED.
      if [[ "$PUBLISHING_TYPE" == "USER_MANAGED" ]]; then
        echo "Deployment ended with status ${DEPLOYMENT_STATE}; publish it in the Central Portal."
        printf '%s\n' "$STATUS_JSON"
        exit 1
      fi
      ;;
