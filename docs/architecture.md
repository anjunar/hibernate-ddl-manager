...
1. **Model coverage for real entities.** The common basic types, enums, `UUID` keys,
   ...
   keys work; an end-to-end test migrates a set of such entities into PostgreSQL. Next: maps
   and ordered lists, then secondary tables and `@Lob`. New types also need a
