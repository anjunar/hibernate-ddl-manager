...
A type change step names its rule, states that every value is kept, and says separately
whether a rewrite of the table and its indexes is possible or expected, with the unique keys,
indexes and check the database rebuilds with the column. Objects outside the model that
depend on the column, such as views, are a required check, the retyped table's size an
optional one. Nothing is tried out: a preview never runs `ALTER COLUMN TYPE`. A dropped index or unique key is bound
by reading, as the migration binds it under its locks: its step shows the SQL with the name the
catalog has, and a required check fails when no single object matches or a foreign key depends
on it. A dropped column or table lists the keys and indexes that go with it. Report format 2
added the type change to the steps.
...
| Diff | New and renamed sequences, new tables, new columns (a required one is added nullable and made required after a NULL count under the lock), columns becoming required or optional, new unique keys and indexes, dropped or replaced unique keys and indexes whose columns remain (replacements first; dropping a unique key needs `Approval.DropUniqueKey` for its signature), new foreign keys (added after all tables, so cycles work), added, changed or removed column checks (existing rows are validated), table and column renames, type widenings that keep every value (a longer `VARCHAR`, `INTEGER` to `BIGINT`, a larger `NUMERIC` precision with the same scale) on columns outside primary keys, foreign keys and identities, with the column's check removed before and set again after; approved drops of columns (with the keys and indexes over them), tables (all in one statement, so they may reference each other) and sequences, run last and without `CASCADE` | Unapproved drops, dropping a foreign key whose columns remain, dropping a unique key without its own approval (a drop or revert approval does not count), reusing a dropped name in the same plan, sequence start or increment changes, identity changes, new identity columns in existing tables, a required column with NULLs left, other type changes (narrowing, another scale or type family, even with approvals) and type changes of key or identity columns, primary key/foreign key changes, schema moves, rename collisions and swaps |
...
| PostgreSQL | Dropped indexes and unique keys bound under the lock, before any DDL, to the one catalog object with exactly their definition, whatever its name; ordinary permanent tables with exactly these columns and `GENERATED BY DEFAULT` identity columns whose sequence has PostgreSQL's defaults (start, increment and minimum 1, the type's maximum, cache 1, no cycling), unowned bigint sequences with default minimum, maximum, cache and no cycling, a non-deferrable primary key, plain unique constraints, plain B-tree indexes and plain foreign keys, matched by structure; column checks, matched by their derived name, column and definition | `GENERATED ALWAYS` identity, identity sequences with other options, sequences with other types or options or owned by a column, other constraints, unexpected or `NOT VALID` checks, checks with another definition, unique, partial, expression, covering or non-B-tree indexes, custom operator classes, collations or `NULLS` ordering, deferrable, `INCLUDE` or `NULLS NOT DISTINCT` unique keys, foreign keys with actions, `MATCH FULL` or deferrable checking, foreign keys from unmodeled tables, triggers, rules, RLS, inheritance, partitions, custom collations; views, rules, triggers, policies and SQL function bodies that depend on a column whose type changes (named under the lock before any DDL, never dropped with `CASCADE`); missing or ambiguous objects to drop, and drops a foreign key depends on |
...
- **Dropping a foreign key whose columns remain**, and changing primary keys.
- **Renaming indexes and constraints**, `CREATE INDEX CONCURRENTLY` and `DROP INDEX
...
- **Type changes beyond the three widenings:** narrowing with an approval and a data check,
  conversions with a declared rule, and coordinated changes of primary keys, foreign keys and
  identity sequences.
- **Data migrations** beyond backfills of NULLs, multi-phase deployments (expand/contract),
